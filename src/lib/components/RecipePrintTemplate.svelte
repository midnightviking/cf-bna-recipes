<script>
	...
	const calculateServings = (_yield, value, unit_name = null) => {
		...
		if (units && unit_name !== null) {
			const unitInfo = units?.find((u) => u.name === unit_name);
			if (
				unitInfo &&
				unitInfo.conversion_unit &&
				unitInfo.conversion_threshold &&
				unitInfo.conversion_formula
			) {
...
>
	...
	<div class="recipe-block">
		<div class="block-header">
			<h4>Critical Control Point (CCP):</h4>
		</div>
		<div class="block-content">
			{recipe.ccp}
		</div>
