						...
						<div class="sections-list">
							{#each section_list as s, si}
								<div class="section-row">
									<input type="text" bind:value={section_list[si].name} placeholder="Section name" />
									<button type="button" title="Remove" onclick={() => { section_list.splice(si,1); section_list=[...section_list]; }}>Remove</button>
								</div>
							{/each}
						</div>
						<div class="add-section">
							<input type="text" bind:value={new_section_name} placeholder="New section name" />
							<button type="button" onclick={() => { if ((new_section_name||'').trim()) { section_list = [...section_list, { name: new_section_name.trim(), ordering: section_list.length }]; new_section_name=''; } }}>Add Section</button>
						</div>
					</div>
				</Cell>
				<!-- Recipe Name Row-->
				...
				<Cell span={12}>
					<div class="recipe-ingredients-section">
						<div class="add-ingredient-row">
							<InnerGrid>
								...
								<Cell  spanDevices={{ desktop: 2, tablet: 4, phone: 2}}>
									<Textfield bind:value={new_quantity} label="Quantity" type="number" min="0" step="2" input$step="2" />
								</Cell>
								<Cell spanDevices={{ desktop: 4, tablet: 4, phone: 2 }}>
									<Select key={(unit) => `${unit ? unit.name : ''}`} bind:value={new_unit} label="Unit">
										{#each units as u}
								...
								<Cell spanDevices={{desktop: 8}}>
									<Select bind:value={new_ingredient_section} key={(s) => `${s ? s.name: ''}`}>
								...
								<Cell span={4} spanDevices={{ desktop: 2, tablet: 12, phone: 4}} align="middle" style="align-content:end;">
									<div class="end-button">
										<Button onclick={addIngredientToRecipe} variant="raised">
											<Icon tag="svg" viewBox="0 0 24 24"><path fill="currentColor" d={mdiPlus} /></Icon>
											Add
										</Button>
									</div>
								</Cell>
						...
						<div class="recipe-ingredient-list">
								...
								<List>
									<div bind:this={section.el} data-section-id={section.id}>
										{#each section.ingredients as ing, index}
											<IngredientItem
												...
												onRemove={(i) => removeIngredientFromRecipe(i,s)}
...
let _localIndex = 0;
let ingredient_list = $state((ingredients ?? []).map(i => ({ ...i, section_id: i.section_id ?? -1, _localIndex: _localIndex++  })));
			...
			section.el,
			{
				...
				store:{
					set:function(sortable){
						...
						let _sid = sortable.el.getAttribute('data-section-id') ?? -1;
						
						section.sorting_group = sorder;
						// console.log(sortable.el)
						section_list.ingredients = [];
						sorder.forEach((s,_i)=>{
							let ing = ingredient_list.find(i => i._localIndex == s);
							if(!ing){
								return
							}
...
const addIngredientToRecipe = () => {
	...
	const sectionIdx = section_list.findIndex(s => s.id === new_ingredient_section);
	if (sectionIdx ) {
		if (!section_list[sectionIdx].ingredients) section_list[sectionIdx].ingredients = [];
		section_list[sectionIdx].ingredients = [...section_list[sectionIdx].ingredients, newIng];
	}
	//reset
	new_ingredient = null;
	new_unit = null;
...
async function saveRecipe() {
	...
	const recipe = {
		...
		sections: section_list.filter((s)=>(s.id > -1 )).map((s, idx) =>  ({ id: s.id, name: s.name, ordering: s.ordering ?? idx })),
...
<style lang="scss">
	...
	.recipe-sections { margin: 1rem 0; }
	.recipe-sections .sections-list { display: flex; flex-direction: column; gap: .5rem; margin: .5rem 0; }
	.recipe-sections .section-row { display: flex; gap: .5rem; align-items: center; }
	.recipe-sections .section-row input { flex: 1; }
	.recipe-sections .add-section { display:flex; gap:.5rem; align-items:center; }
</style>